    ...
    <div className="w-full px-12 max-lg:px-5 h-fit py-2 bg-[#18181B]">
      <div className="flex items-center justify-between w-full h-[10%]">
        ...
        >
          ...
          <h1 className="max-sm:hidden">Actors</h1>
        </Link>
        <TopNev />
      </div>
      <InfiniteScroll
      ...
      >
        <div className="w-full max-sm:gap-5 flex gap-8 h-fit flex-wrap overflow-auto justify-center items-center">
          {trending.map((e, i) => {
            return (
              <Link key={i} to={`/${e.media_type || "person"}/details/${e.id}`}>
                <MainCards data={e} />
              </Link>
            );
          })}
