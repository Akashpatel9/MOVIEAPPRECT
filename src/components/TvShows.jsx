  ...
  return trending.length > 0 ? (
    <div className="w-full px-12 max-lg:px-5 h-fit py-2 bg-[#18181B]">
      <div className="flex items-center justify-between w-full h-[10%]">
        <Link
          to={"/"}
          className=" flex items-center justify-center gap-2 cursor-pointer text-zinc-500 font-extrabold text-2xl max-md:text-xl max-sm:text-sm"
        >
          <i className="ri-arrow-left-line"></i> <h1 className="max-sm:hidden">TvShows</h1>
        </Link>
        <TopNev />
      ...
      >
        <div className="w-full flex gap-8 max-sm:gap-5 h-fit flex-wrap overflow-auto justify-center items-center">
          {trending.map((e, i) => {
            return (
              <Link key={i} to={`/${e.media_type || "tv"}/details/${e.id}`}>
                <MainCards data={e} />
              </Link>
            );
          })}
